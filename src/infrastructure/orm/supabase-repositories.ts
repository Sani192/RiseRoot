...
export const supabaseTaskRepository: TaskRepository = {
  listByDate: async (userId, planDate) =>
    (await supabaseQueries.dailyTasks.listByDate(userId, planDate)).map(
      mapDailyTaskRowToDomain,
    ),
  upsert: async (input) =>
    mapDailyTaskRowToDomain(
      await supabaseQueries.dailyTasks.upsert({
...
export const supabaseScheduleRepository: ScheduleRepository = {
  upsertPlan: async (input) => {
    const r = await supabaseQueries.dailyPlans.upsert({
      user_id: input.userId,
  ...
  listPlanCompletions: (userId, range) =>
    supabaseQueries.calendar.listPlanCompletions(userId, range),
};
export const supabaseWorkoutRepository: WorkoutRepository = {
  listUpcoming: async (userId) =>
    (await supabaseQueries.workouts.listUpcoming(userId)).map(
      mapWorkoutRowToDomain,
    ),
  upsert: async (input) =>
    mapWorkoutRowToDomain(
      await supabaseQueries.workouts.upsert({
        user_id: input.userId,
        name: input.name,
        workout_type: input.workoutType ?? "mixed",
...
export const supabaseWeightRepository: WeightRepository = {
  listByDate: async (userId, loggedOn) =>
    (
      await supabaseQueries.weightLogs.list(userId, {
        from: loggedOn,
        to: loggedOn,
      })
    ).map(mapWeightLogRowToDomain),
  create: async (input) =>
    mapWeightLogRowToDomain(
      await supabaseQueries.weightLogs.create({
        user_id: input.userId,
...
export const supabaseNoteRepository: NoteRepository = {
  upsertByDate: async (input) => {
    const existing = (
      await supabaseQueries.dailyNotes.list(input.userId, {
        from: input.noteDate,
        to: input.noteDate,
      })
    ).find((note) => note.category === "daily");
    const row = existing
      ? await supabaseQueries.dailyNotes.update(existing.id, input.userId, {
          body: input.body,
          daily_plan_id: input.dailyPlanId ?? null,
        })
      : await supabaseQueries.dailyNotes.create({
          user_id: input.userId,
          daily_plan_id: input.dailyPlanId ?? null,
  ...
  findByDate: async (userId, noteDate) => {
    const row = (
      await supabaseQueries.dailyNotes.list(userId, {
        from: noteDate,
        to: noteDate,
      })
    ).find((note) => note.category === "daily");
...
export const supabaseMoodRepository: MoodRepository = {
  create: async (input) =>
    mapMoodLogRowToDomain(
      await supabaseQueries.moodLogs.create({
        user_id: input.userId,
        logged_on: input.loggedOn,
...
export const supabaseReminderRepository: ReminderRepository = {
  create: async (input) =>
    mapReminderRowToDomain(
      await supabaseQueries.reminders.create({
        user_id: input.userId,
        daily_plan_id: input.dailyPlanId ?? null,
        related_type: input.relatedType as ReminderRelatedType | null,
...
export const supabaseUserRepository: UserRepository = {
  ...
  create: async (input) =>
    mapUserRowToDomain(
      await supabaseQueries.users.create({
        ...(input.id ? { id: input.id } : {}),
        display_name: input.displayName ?? null,
        email: input.email ?? null,
        timezone: input.timezone ?? "UTC",
        unit_system: input.unitSystem ?? "imperial",
      }),
    ),
  upsertOnboardingProfile: async () => {
    throw new Error(
      "Supabase onboarding profile persistence is not implemented; use the Drizzle repository path.",
    );
  },
};
