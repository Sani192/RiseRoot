...
import type { UpsertDailyTaskInput, UpsertWorkoutInput } from "@/domain/repositories";

export * from "@/lib/schedule-engine";

function mapPlanTask(userId: string, planId: string, localDate: string, task: DailyPlanTask, sortOrder: number): UpsertDailyTaskInput {
  const recurrenceId = `daily-plan:${task.id}`;
  createGenerationKey(userId, recurrenceId, localDate);
  return { userId, dailyPlanId: planId, title: task.title, description: task.description, sortOrder };
}
...
export async function ensureScheduleForDate(userId: string, at: Date, timeZone: string): Promise<Schedule> {
  ...
  const plan = await scheduleRepository.upsertPlan({ userId, planDate, status: "active", summary: `${planTemplate.focus} ${planTemplate.affirmation}` });

  const tasks = planTemplate.tasks.map((task, idx) => mapPlanTask(userId, plan.id, planDate, task, idx));
  const createdTasks = await Promise.all(tasks.map((task) => taskRepository.upsert(task)));
  ...
  return { planDate, planId: plan.id, generatedTaskIds: createdTasks.map((x) => x.id), generatedWorkoutId: workout.id };
}
