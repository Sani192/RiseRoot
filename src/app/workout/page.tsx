...
export default function WorkoutPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        const workouts =
          (await plannedResourcesService.workouts.list()) as Array<{
            id: string;
            title?: string;
            name?: string;
            status: string;
        ...
        <section className="space-y-5 pb-6" aria-labelledby="workout-heading">
          <Card className="space-y-3 border-white/60 bg-white/75 backdrop-blur-xl">
            ...
            <p className="text-sm leading-6 text-muted-foreground">
              Workout cards are rendered from persisted records.
            </p>
          </Card>
          <Card className="space-y-4 border-white/60 bg-white/75 backdrop-blur-xl">
            ...
            {status === "error" && (
              <p className="text-sm text-red-600">
                Could not load workouts. Sign in or configure local development
            ...
            {status === "ready" && rows.length > 0 && (
              <ul className="space-y-2">
                {rows.map((r) => (
                  ...
                  >
                    <span className="font-semibold">{r.name}</span> · {r.status}
                  </li>
                ))}
              </ul>
            )}
